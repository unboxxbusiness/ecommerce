...
export function CartView({ isPage = false }: { isPage?: boolean }) {
  const { 
    cartItems, 
    removeFromCart, 
    updateQuantity, 
  ...
  const cartBody = (
    <>
      <ScrollArea className={isPage ? "h-auto" : "flex-1"}>
          <div className="divide-y divide-border pr-6">
            {cartItems.map((item) => (
              <div key={item.id} className="flex items-start gap-4 py-4">
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground font-semibold">₹{item.price.toFixed(2)}</p>
        ...
        <div className="border-t p-6">
             <Button className="w-full" asChild onClick={handleCheckout}>
